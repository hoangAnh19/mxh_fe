      ...
      <b-row>
        ...
        <b-col cols="8" v-if="componentManager == 1">
          ...
          <b-row class="time-line" v-if="component == 1">
            <b-col cols="8">
              <TimeLine v-if="member.status==config.member.status.member" v-bind:group="group"  type_="group" />
              <div v-else>Chào mừng bạn đã đến với group, hãy tham gia group để có thể trao đổi với các thành viên trong nhóm</div>
            </b-col>
            <b-col cols="4">
              <IntroGroup />
            </b-col>
          ...
          <b-row v-if="component == 2">
            <IntroduceGroup v-bind:count="count" v-bind:group="group" />
          </b-row>
          <b-row v-if="component == 3">
            <ListMember v-bind:count="count.person" v-bind:member="member" />
          </b-row>
          <b-row v-if="component == 4"> 4 </b-row>
        </b-col>
        <b-col v-if="componentManager == 2">
          <ListPending v-bind:count="count.pending" v-bind:member="member" />
        </b-col>
        <b-col v-if="componentManager == 3">
          <ListPostBrowse v-bind:group_id="group.id"/>
        </b-col>
        <b-col v-if="componentManager == 4">
          <ListPrevent v-bind:count="count.prevent" v-bind:member="member" />
        </b-col>
