  ...
  <div class="form-post" v-if="post.id">
    <div class="d-flex">
      <div class="avatar">
        <img
          ...
          :src="'http://127.0.0.1:8000/tmp_images/' + post.user.avatar"
        />
      ...
      <div class="position-relative info-right">
        ...
        <router-link
          v-if="(post.user_2 ?? {}).id"
          :to="{ name: 'Profile', params: { userId: post.user_2.id } }"
          class="full-name"
        ...
        <br />
        <br />
        <span v-on:click="hiddenTypeShow = false" style="font-size: 12px">
          <router-link
            class="link"
          ...
          <span v-if="post.type_show == 1"
            ><b-icon icon="dot"></b-icon><b-icon icon="globe"></b-icon
          ></span>
          <span v-if="post.type_show == 2"
            ><b-icon icon="dot"></b-icon><b-icon icon="people"></b-icon
          ></span>
          <span v-if="post.type_show == 3"
            ><b-icon icon="dot"></b-icon><b-icon icon="lock"></b-icon
          ></span>
          <span v-if="post.type_show == 4"
            ><b-icon icon="dot"></b-icon><b-icon icon="person"></b-icon
          ></span>
          <span v-if="post.type_show == 5"
            ><b-icon icon="dot"></b-icon><b-icon icon="person-dash"></b-icon
          ></span>
        </span>
      </div>
      <div>
        <b-icon
          v-if="!this.post.user_id_2 || post.user.user_id_2 == this.owner.id"
          icon="three-dots"
    ...
    <div class="main">
          ...
          <div class="d-flex">
            <div class="avatar">
              <img
                ...
                :src="
                  'http://127.0.0.1:8000/tmp_images/' + post.post_share.user.avatar
                "
              />
              <img v-else src="@/assets/image/default-user-avatar.png" />
            </div>
            <div class="position-relative info-right">
              <router-link
                ...
                class="full-name"
                >{{ fullname(post.post_share.user) }}</router-link
              >
              <router-link
                v-if="((post.post_share ?? {}).user_2 ?? {}).id"
                :to="{
                  name: 'Profile',
                  params: { userId: (post.post_share ?? {}).user_2.id },
                }"
                class="full-name"
                ><b-icon icon="caret-right-fill"></b-icon
                >{{ fullname((post.post_share ?? {}).user_2) }}</router-link
              >
              <br />
              <br />
              <span v-on:click="hiddenTypeShow = false" style="font-size: 12px">
                <router-link
                  class="link"
                  :to="{ name: 'Post', params: { postId: post.post_share.id } }"
                >
          ...
          <div class="main">
            ...
            >
              ...
              <div v-if="indexImage > 1" class="next-left">
                <b-icon icon="chevron-left" v-on:click="nextImage(-1)"></b-icon>
              </div>
              <div class="images-wrapper">
                <div class="images" :id="'images' + post.post_share.id">
                  <div
                    class="image"
      ...
      <div class="count d-flex">
        ...
        <div class="share" v-if="share_count" v-on:click="showListShare()">
          {{ share_count }}
          chia sẻ
        </div>
      </div>
      <div class="interactive d-flex">
        ...
        >
          <div style="margin: auto; position: relative">
            ...
            >
              <div class="icon-like" v-on:click.stop v-on:click="like(1)">
                👍
              </div>
              <div class="icon-like" v-on:click.stop v-on:click="like(2)">
                ❤️
              </div>
              <div class="icon-like" v-on:click.stop v-on:click="like(3)">
                😆
              </div>
              <div class="icon-like" v-on:click.stop v-on:click="like(4)">
                😯
              </div>
              <div class="icon-like" v-on:click.stop v-on:click="like(5)">
                😢
              </div>
              <div class="icon-like" v-on:click.stop v-on:click="like(6)">
                😠
              </div>
            </div>
          </div>
        ...
        >
          <div style="margin: auto">
            <b-icon icon="chat" style="font-size: 24px"></b-icon>&nbsp;Bình luận
          </div>
        </div>
        <div class="share_ d-flex" style="flex: 1" v-on:click="modal = true">
          <div style="margin: auto">
            <b-icon icon="share" style="font-size: 24px"></b-icon>&nbsp;Chia sẻ
          </div>
        </div>
      </div>
      <div class="list_comment">
        <div class="form-comment d-flex">
          <div class="avatar_">
            <img
              v-if="owner.avatar"
              :src="'http://127.0.0.1:8000/tmp_images/' + owner.avatar"
            />
            <img v-else src="@/assets/image/default-user-avatar.png" />
          </div>
          <input
        ...
        <div
          style="cursor: pointer"
          v-if="stillComment"
          v-on:click="getComment({ page: page_comment, post_id: post.id })"
        >
          Xem thêm bình luận
        </div>
        <div
        ...
        >
          <div class="avatar_comment">
            <img
              ...
              :src="'http://127.0.0.1:8000/tmp_images/' + comment.user.avatar"
            />
            <img v-else src="@/assets/image/default-user-avatar.png" />
          </div>
          <div>
            ...
            <div class="answer-component">
              <div
                style="
                  ...
                  cursor: pointer;
                "
                v-on:click="comment.isAnswer = !comment.isAnswer"
              >
            ...
            <div
              class="list-answer"
              v-on:click="
                getComment({ page: comment.page, comment_id: comment.id })
              "
            ...
            >
              ...
              >
                <div class="avatar_answer">
                  <img
                    ...
                    :src="'http://127.0.0.1:8000/tmp_images/' + answer.user.avatar"
                  />
                  <img v-else src="@/assets/image/default-user-avatar.png" />
                </div>
                <div>
                  <div class="content_answer">
                    <router-link
                      :to="{
                        name: 'Profile',
                        params: { userId: answer.user.id },
                      }"
                      class="full-name"
                      >{{ fullname(answer.user) }}</router-link
                    ><br />
              ...
              <span v-if="comment.stillAnswer">Xem thêm phản hồi</span>
            </div>
            <div class="d-flex" v-if="comment.isAnswer">
              ...
              <input
                ...
                :placeholder="'Trả lời ' + fullname(comment.user) + '...'"
                @keydown.enter.exact.prevent="sendAnswer(comment)"
              />
            </div>
          </div>
        </div>
    ...
    <div class="modal-dialog1">
      <div class="modal-content1" v-click-outside="closeModal">
        <div class="share-header">
          <span class="fw-bold">Chia sẻ lên dòng thời gian của bạn</span>
        </div>
            ...
            <b-col cols="3">
              ...
              >
                <span v-if="type_show == 1"
                  ><b-icon icon="globe"></b-icon> Công khai</span
                >
                <span v-if="type_show == 2"
                  ><b-icon icon="people"></b-icon> Bạn bè</span
                >
                <span v-if="type_show == 3"
                  ><b-icon icon="lock"></b-icon> Chỉ mình tôi</span
                >
                <span v-if="type_show == 4"
                  ><b-icon icon="person"></b-icon> Bạn bè cụ thể</span
                >
                <span v-if="type_show == 5"
                  ><b-icon icon="person-dash"></b-icon> Bạn bè trừ</span
                >
                <ul
                  v-if="!hiddenTypeShow"
                ...
                >
                  ...
                  <li v-on:click="hideShow(4)" class="list-item">
                    <b-icon icon="person"></b-icon> Bạn bè cụ thể
                  </li>
                  <li v-on:click="hideShow(5)" class="list-item">
                    <b-icon icon="person-dash"></b-icon> Bạn bè trừ
                  </li>
                </ul>
              </button>
            </b-col>
          ...
          >
            <div class="avatar">
              <img
                ...
                :src="'http://127.0.0.1:8000/tmp_images/' + item.user.avatar"
              />
              <img v-else src="@/assets/image/default-user-avatar.png" />
            </div>
            <div class="position-relative info-right">
              <router-link
                :to="{ name: 'Profile', params: { userId: item.user.id } }"
                class="full-name"
      ...
      <div class="modal-content1" v-click-outside="closeModalListShare">
        ...
        <div class="list">
          ...
          >
            ...
            <div class="float:right">
              <router-link :to="{ name: 'Post', params: { postId: item.id } }"
                >Link</router-link
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    loadListShare() {
      ...
      Axios.get(
        "share/get_list?post_id=" + post_id + "&page=" + (this.page_share + 1)
      )
        .then((res) => {
          if (res.data.status == "success") {
            console.log(res.data.data);
    ...
    nextImage(param) {
      ...
      slidechuyen.style = "transform: translateX(" + this.positionX + "px)";
    },
    like(type) {
      Axios.post("like", {
        post_id: this.post.id,
    ...
    getComment(options) {
      ...
      if (options.post_id) {
        this.page_comment++;
        url += "page=" + this.page_comment + "&post_id=" + options.post_id;
      ...
      Axios.get(url)
        .then((response) => {
          if (response.data.status == "success") {
            if (options.post_id) {
              var data = response.data.data;
              this.listComment = data.reverse().concat(this.listComment);
              ...
              if (
                (response.data.data.length < 2 && this.page_comment == 1) ||
                (response.data.data.length < 5 && this.page_comment > 1)
              ) {
              ...
              this.listComment.forEach((comment) => {
                if (comment.id === options.comment_id) {
                  if (!comment.listAnswerComment)
                    comment.listAnswerComment = [];
                  comment.listAnswerComment = comment.listAnswerComment.concat(
                    response.data.data
                  );
                  if (
                    (response.data.data.length < 2 && comment.page == 1) ||
                    (response.data.data.length < 5 && comment.page > 1)
                  ) {
