      ...
      >
        ...
        <div style="flex: 1; margin: auto 10px">
          ...
          <div class="d-flex justify-content-between form-content">
            <div class="last_message" v-html="lastMessage(item)"></div>
            <div class="last_time">
              {{ convertTime((item.last_message ?? {}).created_at) }}
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  left.onscroll = () => {
      if (
        left.scrollTop + left.offsetHeight >= left.scrollHeight && this.ajaxListMessage==false
      ) {
