<template>
  <div class="list-birthday">
    <div v-if="months['yesterday'].length" class="yesterday form-month">
      ...
      >
        <div class="avatar">
          <img
            v-if="friend.avatar"
            :src="'http://127.0.0.1:8000/tmp_images/' + friend.avatar"
          />
        ...
        <div class="right">
          ...
          <div
            ...
            class="form-control"
            @keydown.enter.exact.prevent="sendMessage($event, friend.id)"
            style="width: 100%"
          ></div>
        </div>
      </div>
    ...
    <div v-if="months['now'].length" class="now form-month">
      ...
      <div v-for="(friend, index) in months['now']" :key="index" class="item">
        <div class="avatar">
          <img
            v-if="friend.avatar"
            :src="'http://127.0.0.1:8000/tmp_images/' + friend.avatar"
    ...
    >
      <div v-if="months[index + month - 1].length">
        ...
        <div class="second-title">
          <span class="fw-bold">{{
            months[index + month - 1][0].first_name + " " + months[index + month - 1][0].last_name
          }}</span>
          <span v-if="months[index + month - 1].length - 1">
            và {{ months[index + month - 1].length - 1 }} người khác</span
          >
        </div>
    ...
    <div class="month-next form-month" v-for="index in month - 1" :key="index">
      <div v-if="months[index].length">
        <div class="title h4">Tháng {{ index }}</div>
        <div class="second-title">
          <span class="fw-bold">{{
            months[index][0].first_name + " " + months[index][0].last_name
          }}</span>
        ...
        <div class="list_avatar">
          <div v-for="friend in months[index]" :key="friend" class="avatar_">
            <img
              v-if="friend.avatar"
              :src="'http://127.0.0.1:8000/tmp_images/' + friend.avatar"
              :title="
