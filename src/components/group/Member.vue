  ...
  <div class="d-flex">
    ...
    <div class="control d-flex">
      ...
      <div v-if="isManager && friend_ !== -1">
        ...
        >
          <b-icon class="control_manager" icon="three-dots-vertical"></b-icon>
          <div
            v-if="optionManager"
...
export default {
  ...
  created() {
    if ((this.user["relationship2"] ?? []).length) {
      ...
      if (this.user.id === JSON.parse(localStorage.getItem("userInfo")).id) {
        this.friend_ = -1;
      } else this.friend_ = 0;
    }
  },
    ...
    assign(role) {
      ...
      switch (role) {
        case this.config.group.role.admin:
          confi = confirm("Bạn có muốn cho người này làm quản trị viên");
          break;
        case this.config.group.role.censor:
          confi = confirm("Bạn có muốn cho người này làm kiểm duyệt viên");
          break;
        default:
          confi = confirm("Bạn có muốn gỡ bỏ quyền hiện tại của người này");
          break;
      }
      if (confi) {
