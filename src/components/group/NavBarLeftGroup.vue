<template>
  <div class="left-groups">
    ...
    <div class="manager" v-if="listGroupManager.length">
      <div class="title-1">
        Nhóm bạn quản lý
        <span class="view-all" v-on:click="changeComponent(3)">Xem tất cả</span>
      </div>
        ...
        >
          <div class="cover">
            <img
              ...
              :src="
                'http://127.0.0.1:8000/tmp_images/' + listGroupManager[item - 1].cover
              "
            />
          ...
          <div class="mx-2" style="flex: 1">
            <router-link
              :to="{ name: 'Group', params: { groupId: listGroupManager[item - 1].id } }"
              class="user-name fw-bold"
            >
              {{listGroupManager[item - 1].name}}
            </router-link>
            <div>{{listGroupManager[item - 1].count_member}} thành viên</div>
          </div>
        </div>
      </div>
    </div>
    <div class="participation" v-if="listGroupNomarl.length">
      <div class="title-1">
        Nhóm bạn tham gia
        <span class="view-all" v-on:click="changeComponent(4)">Xem tất cả</span>
      </div>
        ...
        >
          <div class="cover">
            <img
              ...
              :src="
                'http://127.0.0.1:8000/tmp_images/' + listGroupNomarl[item - 1].cover
              "
            />
            <img v-else src="@/assets/image/default-user-avatar.png" />
          </div>
          <div class="mx-2" style="flex: 1">
            <router-link
              :to="{ name: 'Group', params: { groupId: listGroupNomarl[item - 1].id } }"
              class="user-name fw-bold"
            >
              {{listGroupNomarl[item - 1].name}}
            </router-link>
            <div>{{listGroupNomarl[item - 1].count_member}} thành viên</div>
          </div>
        </div>
      </div>
    </div>
...
.group_normarl_item, .group_manager_item {
  margin-bottom: 20px ;
}
